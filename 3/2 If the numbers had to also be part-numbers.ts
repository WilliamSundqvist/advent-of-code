...
for (var [y, line]  of bigSchema.entries())
{
   for (var [x, square] of line.entries())
   {
    ...
    if (gear.isGear)
    {
        getGearNeighbors(x, y, bigSchema);
    }
   } 
}
...
function markNeighbors(x: number, y: number, schema: Square[][])
{
    ...
    for (const neighbor of neighborsToMark)
    {
        ...
        console.log(schema[neighbor.Y][neighbor.X]);
    }
}
...
function getGearNeighbors(x: number, y: number, schema: Square[][])
{
    //above = y-1; x-1, x, x+1
    //sides = y; x-1, x+1
    //below = y+1; x-1, x, x+1
...
function getNeighbor(square: Square, gear: Gear) 
{
    ...
    if (!part.adjacent)
        return;
    console.log(part);
    console.log(gear.neighbors);
    if (gear.neighbors.find((p) => p.id == p.id))
        return;
